...
module Jekyll
  class PlaygroundTag < Liquid::Tag
    ...
    def render(context)
      ...
      <<~HTML
        <section id="pg-#{id}" class="playground" aria-label="Code playground: #{name}"
                 data-playground data-id="#{id}" data-autorun="#{autorun}">
      ...
      <<~HTML
        <header class="playground-header">
          <div class="playground-brand">
            ...
            <h2 class="playground-title" id="pg-title-#{id}">
    ...
    def render_main_content(playground, context, id, name, autorun)
      <<~HTML
        <div class="playground-content" data-split-pane role="group" aria-label="Playground workspace">
          <!-- Editor Section -->
          <section class="editor-section" data-pane="left" role="region" aria-labelledby="pg-editors-label-#{id}">
            <h3 id="pg-editors-label-#{id}" class="visually-hidden">Code Editors</h3>
          ...
          <!-- Preview Section -->
          <section class="preview-section" data-pane="right" role="region" aria-labelledby="pg-preview-label-#{id}">
            <h3 id="pg-preview-label-#{id}" class="visually-hidden">Preview and Console</h3>
        ...
        <<~HTML
          <div class="editor-panel" id="#{panel_id}" role="tabpanel" aria-labelledby="#{tab_id}"
            ...
            <div class="code-editor">
              <div class="cp-gutters">
                <div class="cp-gutter cp-lines"></div>
              </div>
              <div id="code-#{id}-#{file['type']}-#{idx}" contenteditable="true" spellcheck="false" autocorrect="off" autocapitalize="off" writingsuggestions="false" translate="no" role="textbox" aria-label="Code editor for #{file["name"]}" aria-multiline="true" aria-describedby="pg-title-#{id}">#{escape_html(file["code"]).strip}</div>
            </div>
    ...
    def render_console_buttons
      %w[log info debug warn error].map do |level|
        <<~HTML
          <button class="console-count-#{level}" data-level="#{level}" title="#{level.capitalize} count">
