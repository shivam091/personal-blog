...
If basic shapes like circles and rectangles are the "Lego bricks" of SVG, the `<path>` element is the **professional pen tool**. It is a single element capable of drawing any shape imaginable by following a string of specific commands.

In this post, we’ll decode the "alphabet" of the `d` attribute and learn how to move from straight lines to elegant curves.
...
## Line Command Best Practices

- Always start with `M`
- Use `Z` for closed shapes
- Prefer `H` and `V` for straight edges
- Use relative commands for repetitive patterns
- Keep strokes visible during debugging (`fill="none"`)
...
{% codeblock %}
{% highlight xml linenos %}
<path d="M20 80 Q100 10, 180 80 T340 80"
...
3. Plan **control points** for curves visually.
4. Use **relative commands** for repeated patterns.
5. Use `fill-rule="evenodd"` for overlapping complex shapes.
