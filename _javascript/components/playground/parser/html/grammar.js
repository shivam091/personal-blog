  ...
  rules: {
    ...
    Comment(p) {
      ...
      while (!p.eof()) {
        ...
        p.next(); // Consume COMMENT_TEXT or other internal tokens
      }

      if (!commentClose) {
        p.error(`Unclosed HTML comment: Expected '${htmlTokens.commentEnd}'`, commentOpen);
      }
    ...
    Element(p) {
      ...
      if (isVoid) {
        ...
        const next = p.peek();
        if (next && next.type === "TAG_CLOSE") {
          const closeName = next.value.match(/<\/([A-Za-z0-9]+)>/)?.[1]?.toLowerCase();
          if (closeName === openTagName) {
            p.error(`Void element <${openTagName}> must not have a closing tag.`, next);
            p.next(); // consume erroneous closing tag
          }
        }
      ...
      if (tagClose) {
        ...
        if (openTagName && closeTagName && openTagName !== closeTagName || isVoid) {
          // Backtrack and return null (Prevents bad fold node)
          p.pos = startPos;
          return null;
        }
      }
