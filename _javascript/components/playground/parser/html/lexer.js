...
export class HtmlLexer extends BaseLexer {
  ...
  run() {
    ...
    const peekForTagName = (start) => {
        ...
        return this.tokenSet.tags.has(ident.toLowerCase()) ? ident : null;
    };
    ...
    while (i < L) {
      ...
      if (s[i] === '&') {
        ...
        if (s[j] === '#') {
          ...
          if (entityText.length > 0 && this.tokenSet.entities.has(entityText.toLowerCase())) isValid = true;
        }
      ...
      if (/[A-Za-z0-9_:\-]/.test(s[i])) {
        ...
        if (prevToken && (prevToken.value === '<' || prevToken.value === '</')) {
          const isKnownTag = this.tokenSet.tags.has(lower);
        ...
        if (isInsideTag) {
          const isKnownAttr = this.tokenSet.attributes.has(lower) || isDataAttr(lower) || isAriaAttr(lower);
