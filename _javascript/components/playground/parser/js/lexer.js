  ...
  run() {
    ...
    while (i < L) {
      ...
      if (isIdentStart(ch)) {
        ...
        if (this.tokenSet.keywords.has(v)) {
          type = 'KEYWORD';
          cssClass = 'cp-token-keyword';
        } else if (this.tokenSet.literals.has(v)) {
          type = 'LITERAL';
          cssClass = 'tok-literal';
        } else if (this.tokenSet.builtInVariables.has(v)) {
          type = 'BUILT_IN_VAR';
        ...
        } else if (this.tokenSet.builtInGlobals.has(v) || this.tokenSet.errorTypes.has(v)) {
