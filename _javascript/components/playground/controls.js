/*
 * Responsible for establishing a declarative link between interactive DOM elements
...
export class Controls {
   ...
   *
   * It manages binding click and change events for control elements (buttons/checkboxes)
   ...
   * It executes the action handler and synchronizes the state to the closest ancestor with
   * class `.pen`.
   */
  #handleCheckboxChange = (event) => {
    ...
    // Execute the action handler, passing the boolean state
    if (typeof this.actions[actionName] === "function") {
      this.actions[actionName](checked);
    }

    // Sync state to the closest container (e.g., .pen)
