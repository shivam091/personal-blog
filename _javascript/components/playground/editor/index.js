...
export class Editor {
  ...
  // The element containing the gutter elements
  gutterEl;
  ...
  constructor(root, uniqueKey, meta = {}) {
    ...
    this.gutter = new EditorGutter(this);
    ...
    this.gutter.toggleGutterVisibility(initialGutterVisibleAttr === "on");
  ...
  updateValue(val) {
    ...
    // Sync the cache immediately after HTML is set
    this.syncLineCache();

    // Restore caret
    if (beforeMeta) this.inputHandler.restoreCaretAt(newCaretPos);

    // Cursor + gutter updates
  ...
  /*
   * Synchronizes the high-performance element registry (lineMap).
   * This method iterates through the editor's immediate children once to map
   * line numbers (from data-line attributes) to their corresponding DOM elements.
   * By caching these references in a Map, the editor achieves O(1) access to any
   * line, bypassing expensive DOM lookups (querySelector) during frequent
   * operations like scrolling, folding, and search-highlighting.
   */
  syncLineCache() {
    // Clear the existing entries via the getter
    this.state.lineMap.clear();

    // Populate using the data-line attribute as the key
    for (const child of this.editable.children) {
      const lineNum = parseInt(child.dataset.line, 10);

      if (lineNum) this.state.lineMap.set(lineNum, child);
    }
  }
